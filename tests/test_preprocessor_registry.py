...
from sam_ml.preprocessing.preprocess_ddr2019_dualfilters import (
    Ddr2019DualFiltersPreprocessor,
)
...
class TestPreprocessorRegistry:
    ...
    def test_list_preprocessors_includes_ddr2019(self) -> None:
        ...
        assert "ddr2019_dualfilters" in preprocessors
    ...
    def test_get_preprocessor_ddr2019_dualfilters(self) -> None:
        """get_preprocessor('ddr2019_dualfilters') returns expected class."""
        cls = get_preprocessor("ddr2019_dualfilters")
        assert cls is Ddr2019DualFiltersPreprocessor
...
class TestMiddlewareRegistry:
    ...
    def test_list_middlewares_includes_builtins(self) -> None:
        """Registry includes default, paper_dual, dual_filters_multisize, resize_norm."""
        middlewares = list_middlewares()
        assert "default" in middlewares
        assert "paper_dual" in middlewares
        assert "dual_filters_multisize" in middlewares
